...
function AllFilmsPage() {
    const {rating, year, films, preloaderState} = useTypedSelector(state => state.allFilms)
    ...
    return <div className="allFilmsPage">
        <div className="allFilmsPage__menu">
            <div className="allFilmsPage__menu__header">
                <Typography.Title>Все фильмы</Typography.Title>
                <Typography.Text className="allFilmsPage__menu__header__description">Подборка фильмов</Typography.Text>
            </div>
            <div className="allFilmsPage__menu__filter">
                <Filters name='Рейтинг' maxAndMin={[RATING__MIN, RATING__MAX]} setFiltersValue={setRating}/>
                <Filters name='Год' maxAndMin={[YEAR__MIN, YEAR__MAX]} setFiltersValue={setYear}/>
                <div className="allFilmsPage__menu__filter__btns">
                    <Button type='primary' size='middle' style={{ background: "#1f1d1d", color: "white" }} onClick={getFilmWithFilters}>Применить</Button>
                    <Button type='primary' size='middle' style={{ background: "#1f1d1d", color: "white" }} onClick={getFilmWithoutFilters}>Сброс</Button>
                </div>
            </div>
        </div>
